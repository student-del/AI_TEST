...
每轮 API 调用前，按固定优先级从 9 个来源组装上下文：

| # | 来源 | 说明 | 存储位置 | 持久性 |
|---|------|------|---------|--------|
| 1 | **系统提示** | 角色定义、行为约束、20 种工具列表 | `cli.js` 二进制内（~几千 token） | 版本级固定 |
| 2 | **环境信息** | OS、Shell、工作目录、日期 | 运行时动态获取 | 每轮动态 |
| 3 | **CLAUDE.md 层级** | 4 级合并后的项目指导 | `E:\AI_TEST\CLAUDE.md`（项目级）<br>`~/.claude/CLAUDE.md`（用户级，当前不存在） | 文件持久 |
| 4 | **路径作用域规则** | 文件访问范围 | 运行时 | 会话级 |
| 5 | **自动记忆** | 跨会话共享的用户偏好 | `~/.claude/projects/E--AI-TEST/memory/` | 文件持久 |
| 6 | **工具元数据** | 20 种工具的完整 JSON Schema | `cli.js` 二进制内（~3000 tokens） | 版本级固定 |
| 7 | **对话历史** | 当前会话的历史（**压缩后视图，非完整 JSONL**） | `~/.claude/projects/E--AI-TEST/<sessionId>.jsonl`（源） | 文件持久，但注入时被压缩 |
| 8 | **工具结果** | 最新一轮工具执行的返回值 | 同上 JSONL（小结果内联）<br>`.../tool-results/`（超大结果磁盘缓存） | 同上 |
| 9 | **压缩摘要** | 早期对话被 Auto-Compact 压缩后的摘要 | 同上 JSONL，以 `compact_boundary` 系统消息形式写入 | 同上 |

### 关键细节

**来源 7 对话历史不是完整的 JSONL 文件**。JSONL 保存完整原始记录（当前会话 262 行，~440KB），但注入上下文的实际内容经过了 5 阶段压缩管线处理——Microcompact 清除了旧的 tool result，Context Collapse 可能折叠了早期对话，Auto-Compact 可能将早期历史替换为摘要。压缩后的实际上下文**不存储在任何本地文件中**，是 API 请求发送前在内存中的瞬时产物。

**来源 1 和 6 在 cli.js 中**。系统提示和工具元数据不是配置文件，而是编译在 13MB 的 Bun 二进制产物里，版本更新才会变。

**来源 3 和 5 是概率性约束**。它们被注入为 `role: "user"` 的用户上下文（非 `role: "system"` 的系统提示），模型自行决定遵守程度。需要确定性约束时用权限规则（`settings.local.json`）——代码强制拦截，模型没机会违反。

**来源 9 和 7 存储在同一份 JSONL 中**。Auto-Compact 生成的压缩摘要作为 `type: "system"` 消息追加写入同一个 JSONL 转录文件，没有独立存储。压缩前后的 token 数记录在 `compact_boundary` 标记中。

### 5 阶段压缩管线

上下文窗口有限，需要分层压缩。详见 `../Agent核心机制/上下文压缩机制详解.md`：
...
**关键设计**：Microcompact 利用了 Anthropic API 的 Prompt Caching，通过 `cache_edits` 在服务器端透明删除旧 tool result，保持本地消息前缀不变以保护缓存命中（90% 折扣）。
...
```
确定性  ↑
        │  ① 托管级: /etc/claude-code/CLAUDE.md     ← IT 管理员统一配置，个人不可改
        │  ② 用户级: ~/.claude/CLAUDE.md            ← 跨所有项目生效
        │  ③ 项目级: CLAUDE.md / .claude/CLAUDE.md  ← 跟仓库走，团队共享
        │  ④ 本地级: CLAUDE.local.md               ← 个人偏好，gitignored
        ↓     .claude/rules/*.md                    ← ③ 的子集，规则拆分
概率性  ↓
```
...
越往上越强制，越往下越灵活。多个文件按层级合并，级别越高优先级越高。

### 为什么是概率性遵守

因为 CLAUDE.md 被注入为**用户上下文（`role: "user"`）**，不是**系统提示（`role: "system"`）**。两者有本质区别：

```
系统提示（确定性）:
  role: "system"
  → "你的名字是 Claude Code，你只能使用给定的工具..."
  → 这是硬约束，模型把它当作"你是谁"的定义

CLAUDE.md（概率性）:
  role: "user"
  → "本文件为 Claude Code 在此仓库中工作时提供指导：对话记录必须本地化..."
  → 这是"一段建议文本"，和其他用户消息混在一起
  → 模型读完后自行判断要不要遵守
```

打个比方：系统提示是"你是医生，必须遵守希波克拉底誓言"。CLAUDE.md 是同事给你的一张便条"这个项目用 TypeScript，记得写测试"。你会尽量遵守便条上的内容，但如果遇到特殊情况可能自己做判断。而"你是医生"这个身份定义你不能自己推翻。

### 如果需要确定性约束

用**权限规则**（`settings.local.json`），不是 CLAUDE.md。权限规则是被 CLI 代码强行拦截的，模型根本没机会违反：
...
模型想执行 `rm -rf /` → CLI 不等模型想不想，直接拒绝。

**一句话**：CLAUDE.md 是"建议"，权限规则是"法律"。一个靠模型自觉，一个靠代码强制。
...
| | CLAUDE.md | 自动记忆 |
|---|---|---|
| **注入方式** | `role: "user"` 用户上下文（概率性） | `role: "user"` 用户上下文（概率性） |
| **加载策略** | **全量加载**——4 级合并后全部注入 | **按需加载**——LLM 自己选相关的 |
| **作用范围** | 当前项目 / 当前用户 | 跨会话共享 |
| **更新方式** | 手动编辑 | Claude Code 自动写入 |

本质上都是"一段文本建议"，都不是确定性约束。区别在于加载策略和作用范围。

### 检索方式：LLM 扫描索引，不是向量检索

没有 embedding，没有向量数据库。流程极其简单：

```
会话开始时:
  1. 把 MEMORY.md 索引注入上下文（就那几行）
     → "用户语言偏好 — 用户要求所有回答使用简体中文"
     → "对话记录本地化 — 所有技术讨论和分析结论必须保存为本地文件"

  2. LLM 扫描索引 → 根据每条 description 判断相关性
     "用户语言偏好 → 和当前对话相关吗？用户刚说了中文 → 相关 ✓"
     "对话记录本地化 → 用户要求总结保存 → 相关 ✓"
...
记忆本身也消耗上下文 token。每条记忆可能几百到上千 token，如果 20 条全加载可能直接吃掉几千 token。5 条是一个工程权衡——能覆盖绝大多数场景（很少有一轮对话需要同时参考 6 条以上不相关的记忆），又能把记忆的总 token 预算控制在合理范围（~1000-2000 tokens）。

这就是"文件即记忆"的设计优势：完全透明，你直接打开 `MEMORY.md` 就能看到我看到哪些记忆、每条记忆的 description 是什么、我会基于什么做选择。
